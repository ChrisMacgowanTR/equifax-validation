...
class DbOrganizationEmail:
    ...
    def __init__(self):
        logging.debug('Inside: DbOrganizationEmail::DbOrganizationEmail()')
    ...
    def validation(self, row, results):
        ...
        try:
            ...
            postgreSQL_select_Query = f"SELECT * FROM troa_dev.organization_email where partitionid = '{file_partitionid_in}' AND partition = '{partition_In}'"
            ...
            for row in mobile_records:
                ...
                logging.debug("db_email = %s", db_email)
        ...
        if count > 0:
            ...
            logging.info("Test Class: DbOrganizationEmail")
            logging.info("Test Method: validation()")
            logging.info("Target Database: troa_dev.organization_email")
            ...
            # ------------
            file_EFX_EMAIL_in = file_EFX_EMAIL_in.strip()
            db_email = db_email.strip()

            try:
                assert file_EFX_EMAIL_in == db_email
                assert_message = f"Test Successful - file_EFX_EMAIL_in: {file_EFX_EMAIL_in} is equal to db_email: {db_email}"
                logging.info(assert_message)
                results.add_to_passed()
            except:
                assert_message = f"Test Failed - file_EFX_EMAIL_in: {file_EFX_EMAIL_in} Not Equal db_email: {db_email}"
                logging.info(assert_message)
                results.add_to_failed()
